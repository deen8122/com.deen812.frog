...
public class Rope_Bad extends BoxObject  {
	...
	public Rope_Bad(final float x,final float y) {
		
		S1 = new Sprite (x, y , mRes.h_rope1);
		S2 = new Sprite (x, y , mRes.h_rope_bad1){
			@Override
		...
		B1.getFixtureList().get(0).setSensor(true);
		 ...
		 B2.getFixtureList().get(0).setUserData("kaktus");
		...
		DistanceJointDef djf = new DistanceJointDef();
  	    ...
  	    djf.dampingRatio  = 0.8f;
  	  	djf.frequencyHz =0.4f;
  	  ...
  	  S3 = new Sprite (x+20, y+5 , mRes.h_rope3); 
  	  S3.setRotationCenter(0, 5);
  	  S3.setRotation(90);
