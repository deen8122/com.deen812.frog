...
public class ThanksScene {
   ...
   //  mScene.attachChild(btn_exit21);
     ...
     String str=(String)game.getResources().getString(R.string.thanks_title);            
     Text t = new Text(260, 50, mRes.mFont_Plok, str);
     t.setColor(0.1f, 0.3f, 0.12f);
 	 this.mScene.attachChild(t);
 	 
 	 str=(String)game.getResources().getString(R.string.thanks_text);            
     ...
     Sprite btn_start  = new Sprite(xCenter , 300, mRes.txt_start) {
   		 @Override
            public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float X, float Y)  {	 
                if (pSceneTouchEvent.isActionDown()     ) {
               	 this.setScale((float) 1.2);
