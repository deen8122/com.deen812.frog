...
public class SelectScene {
	...
	private ArrayList<AnimatedSprite> arr_list_as;
	...
	private int mMaxLevelReached = 0;
 ...
 */
    private void CreateLevelBoxes() {
    	...
    	arr_list_as= new ArrayList<AnimatedSprite>();
        ...
        for (int y = 0; y < totalRows; y++) {
                for (int x = 0; x < LEVEL_COLUMNS_PER_SCREEN; x++) {
                         ...
                         AnimatedSprite  Bspr = new AnimatedSprite(boxX, boxY, mRes.menu_level.clone()){
                                ...
                                public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {                                	
                                	...
                                	if(pSceneTouchEvent.isActionDown()){
                                			...
                                			if(mConst.SELECTED_MAP >= mConst.FREE_LEVELS && mConst.FULL_VERSION != 1) {
                                				        game.sceneManager.setCurrentScene(AllScenes.BYE_DEMO);
                                			}
                                			else    game.sceneManager.setCurrentScene(AllScenes.GAME); 
                                      }
                                      if (pSceneTouchEvent.isActionUp() || pSceneTouchEvent.isActionCancel()){
                                      }
                        ...
                        //this.mScene.attachChild(Bspr);
                            ...
                            final AnimatedSprite level_star = new AnimatedSprite(29,75, mRes.level_star.clone());
                            Bspr.attachChild(level_star);
                            arr_list_as.add(level_star);  
                        if (iLevel >  mMaxLevelReached){ 
                        	       arr_list.add(Bspr);   
                        	       Bspr.setCurrentTileIndex(0);
                        	       level_star.setVisible(false);
                        	       level_star.setIgnoreUpdate (true);
                        	...
                        	if(iLevel ==  mMaxLevelReached) {
                        		 ...
                        		 level_star.setCurrentTileIndex(mConst.LEVEL_STAR_COL[ iLevel ]);
                        	}
 ...
 //layer.get(3).setPosition(-10000, -1000);
    }//
	...
	public void updateLevels() {
		...
		if(mConst.LEVEL > mMaxLevelReached) {
			...
			for(int i =0; i < n; i++) {
				...
				AnimatedSprite s = (AnimatedSprite)arr_list.get(i).getChild(0); 
				s.setCurrentTileIndex( mConst.LEVEL_STAR_COL[ i ] );
		...
		for( int i = 0; i < mConst.LEVEL - 1;i++) {
			arr_list_as.get(i).setVisible(true);
			arr_list_as.get(i).setCurrentTileIndex(mConst.LEVEL_STAR_COL[ i ]);
		}
	}
}
